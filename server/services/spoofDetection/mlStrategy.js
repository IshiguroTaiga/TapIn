...
const { checkSpeed, checkAccuracy, checkTimestamp, checkMotionMismatch } = require('./heuristics');
...
class MLStrategy {
   ...
   */
  extractFeatures(currentTrace, history = []) {
  ...
  evaluate(currentTrace, history = []) {
    const { features, heuristicFlags, heuristicDetails } = this.extractFeatures(currentTrace, history);
