...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('attendance_updated', (data) => {
      console.log('[Socket.io] Real-time attendance update received:', data);
      fetchActiveEventData();
    });
  ...
  const fetchActiveEventData = async () => {
    try {
      const eventRes = await axios.get('/api/events/active');
      const event = eventRes.data;
      setActiveEvent(event);

      if (event) {
        const liveRes = await axios.get(`/api/attendance/live/${event.id}`);
    ...
    <div className="w-full max-w-7xl mx-auto px-1 sm:px-4 py-4 sm:py-8 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Live Attendance & Geofence Feed</h1>
          </div>
          <p className="text-xs text-slate-400">Real-time telemetry stream for active university events</p>
        </div>

        <button
          onClick={fetchActiveEventData}
          className="px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-300 hover:text-white flex items-center gap-2 self-start sm:self-auto transition-colors cursor-pointer min-h-[44px]"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="glass-panel p-4 rounded-2xl border border-slate-800 space-y-1">
          ...
          <div className="text-2xl font-bold text-white">{summary.totalActive}</div>
        ...
        <div className="glass-panel p-4 rounded-2xl border border-emerald-500/20 bg-emerald-950/20 space-y-1">
          ...
          <div className="text-2xl font-bold text-emerald-400">{summary.inRangeCount}</div>
        ...
        <div className="glass-panel p-4 rounded-2xl border border-amber-500/20 bg-amber-950/20 space-y-1">
          ...
          <div className="text-2xl font-bold text-amber-400">{summary.outRangeCount}</div>
          <span className="text-[10px] text-amber-500/80">Grace period active</span>
        </div>

        <div className="glass-panel p-4 rounded-2xl border border-rose-500/20 bg-rose-950/20 space-y-1">
          <div className="flex items-center justify-between text-rose-400 text-xs font-medium">
            <span>Spoof Alerts & Flagged</span>
            <ShieldAlert className="w-4 h-4" />
          </div>
          <div className="text-2xl font-bold text-rose-400">{summary.flaggedCount}</div>
